...
mod tests {
    ...
    fn test_translate_aequi_tax_estimate() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "aequi_estimate_quarterly_tax");
    ...
    fn test_translate_aequi_schedule_c() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "aequi_schedule_c_preview");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("2026"));
    }
    ...
    fn test_translate_aequi_import_bank() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "aequi_import_bank_statement");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("/tmp/march.ofx"));
        assert_eq!(t.permission, PermissionLevel::SystemWrite);
    }
    ...
    fn test_translate_aequi_balance() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "aequi_account_balances");
    }
    ...
    fn test_translate_aequi_receipts() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "aequi_list_receipts");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("pending_review"));
    }
    ...
    fn test_translate_task_list() {
        ...
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("in_progress"));
    }
    ...
    fn test_translate_task_create() {
        ...
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("fix bug"));
        assert!(args_str.contains("high"));
    }
    ...
    fn test_translate_task_update() {
        ...
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("abc"));
        assert!(args_str.contains("done"));
    }
    ...
    fn test_translate_delta_create_repo() {
        ...
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("test-repo"));
        assert_eq!(t.permission, PermissionLevel::SystemWrite);
    }
    ...
    fn test_translate_agnostic_submit_task() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "agnostic_submit_task");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("regression"));
        assert_eq!(t.permission, PermissionLevel::SystemWrite);
    }
    ...
    fn test_translate_agnostic_task_status() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "agnostic_task_status");
        assert_eq!(t.permission, PermissionLevel::Safe);
    }
    ...
    fn test_translate_agnostic_list_presets() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "agnostic_list_presets");
        assert_eq!(t.permission, PermissionLevel::Safe);
    }
    ...
    fn test_translate_agnostic_agent_status() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "agnostic_agent_status");
        assert_eq!(t.permission, PermissionLevel::Safe);
    }
    ...
    fn test_translate_edge_list() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "edge_list");
    }
    ...
    fn test_translate_edge_deploy() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "edge_deploy");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("my-task"));
    }
    ...
    fn test_translate_edge_decommission() {
        ...
        let t = interpreter.translate(&intent).unwrap();
        let mcp = t.mcp.as_ref().expect("expected MCP call");
        assert_eq!(mcp.tool_name, "edge_decommission");
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("node-1"));
    }
    ...
    fn test_translate_shruti_mixer() {
        ...
        let args_str = mcp.arguments.to_string();
        assert!(args_str.contains("vocals"));
    }
