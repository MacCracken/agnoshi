...
pub enum Intent {
    ...
    /// Get overall security status from t-ron
    TronStatus,
    ...
    /// Load or reload security policy
    TronPolicy {
        /// TOML policy content
        toml: String,
    },
...
/// MCP tool call metadata — used for native JSON-RPC dispatch via bote.
#[derive(Debug, Clone)]
pub struct McpCall {
    /// Tool name (e.g. "synapse_deploy", "delta_status").
...
pub struct Translation {
    ...
    /// If set, this translation is an MCP tool call dispatched natively via bote
