...
use crate::security::{SecurityContext, analyze_command_permission};
use crate::ui::Ui;
...
impl Session {
    ...
    /// Execute a translation — routes MCP calls natively, shell commands via subprocess.
    async fn execute_translation(
        &mut self,
        translation: &crate::interpreter::Translation,
    ) -> Result<()> {
        if let Some(mcp) = &translation.mcp {
            self.execute_mcp_call(&mcp.tool_name, &mcp.arguments).await
        } else {
    ...
    /// Execute an MCP tool call natively via bote JSON-RPC over HTTP.
    async fn execute_mcp_call(
        &mut self,
        tool_name: &str,
        arguments: &serde_json::Value,
    ) -> Result<()> {
        let request = bote::JsonRpcRequest::new(1, "tools/call").with_params(serde_json::json!({
            "name": tool_name,
            "arguments": arguments,
        }));
        ...
        match resp {
            Ok(response) => {
                ...
                if status.is_success() {
                    match serde_json::from_str::<bote::JsonRpcResponse>(&body) {
                        Ok(rpc_resp) => {
                            if let Some(result) = &rpc_resp.result {
                                // Extract text from MCP content blocks if present.
                                if let Some(content) =
                                ...
                                self.prompt_context.last_exit_code = 0;
                            } else if let Some(err) = &rpc_resp.error {
                                self.ui.show_error(&err.message);
                                self.prompt_context.last_exit_code = 1;
                            }
                        }
                        Err(e) => {
                            // Not JSON-RPC — show raw body.
                            if body.is_empty() {
                                self.ui.show_error(&format!("MCP parse error: {e}"));
                            } else {
                                self.ui.show_output(&body);
                            }
                            self.prompt_context.last_exit_code = 0;
                        }
                    }
                    ...
                    self.ui
                        .show_error(&format!("MCP call failed (HTTP {}): {}", status, body));
                    self.prompt_context.last_exit_code = 1;
                }
            }
            Err(e) => {
                self.ui.show_error(&format!("MCP connection failed: {e}"));
                self.prompt_context.last_exit_code = 1;
            }
        }
