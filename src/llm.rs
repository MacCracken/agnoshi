...
//!
//! Telemetry bridge: when the `agnostik-telemetry` feature is enabled,
//! trace context is generated by `agnostik::telemetry::TraceContext`.
//! Otherwise, a lightweight inline W3C traceparent is used.
...
///
/// When `agnostik-telemetry` is enabled, delegates to `agnostik::telemetry::TraceContext`
/// for ecosystem-consistent tracing. Otherwise, generates a standalone traceparent.
#[cfg(feature = "agnostik-telemetry")]
fn generate_traceparent() -> String {
    let ctx = agnostik::telemetry::TraceContext::new();
    ctx.to_traceparent()
}

#[cfg(not(feature = "agnostik-telemetry"))]
fn generate_traceparent() -> String {
