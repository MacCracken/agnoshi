...
> **Last refresh**: 2026-09-23 — **incremental, against the 1.9.13 tree** (cyrius 6.6.6). Rows
> touched by 1.9.13 are updated below; the rest carry forward.
>
...
|---|---|---|---|
| `README.md` | 2026-09-23 | ✅ Fresh | Stat-line at 1.9.13 (cyrius 6.6.6; 686 unit / 26 security / 92 smoke; 197 KB x86_64 DCE / 661 KB aarch64, with the aarch64 DCE caveat). ⚠ Corrected: `src/approval.cyr` **is** in the include graph (since May) — only its risk classifier is used; the prompt has no caller. Roadmap citations now name arcs. **Feature list corrected**: approval workflows and checkpoint/undo moved out of "Features" into an explicit not-shipped note. **Key-protections list corrected** — escape sanitization, env whitelist and sudo re-verification were listed as active and are not. Module tree rewritten to split **COMPILED** from **PRESENT BUT NOT IN THE BINARY**, which is the root cause of the whole class. |
...
| `SECURITY.md` | 2026-09-23 | ✅ Fresh | Roadmap citations moved from `Bucket 1 Slice N` to arc slots (1.9.13). Supported-versions table 1.0.x → 1.9.x. Four sections marked **⚠ NOT IN THE SHIPPED BINARY** (approval, checkpoint/undo, privilege escalation, the env whitelist) with what actually happens instead — including that AGNOS children *inherit* the environment, the opposite of a whitelist. OWASP section rewritten: ASI01 and ASI03 are now marked not-applicable/not-implemented rather than claimed. |
| `CONTRIBUTING.md` | 2026-09-23 | ✅ Fresh | 1.9.13: the fmt gate is one multi-file `cyrius fmt --check` (`scripts/check-fmt.sh` retired — cyrius 6.6.5 fixed the file-list bug), pin literal at 6.6.6. Gate list completed (`lint-cstr-str.sh`, `check-coverage.sh`) **with the linter's literal-only blind spot stated** — a clean run is not proof. 1.9.10 added lint **Category H** (the audit-path test seam must not be called from `src/`) and fixed the coverage gate's comment-mention blind spot; both noted in § Gates. Sanitizer rule now distinguishes cstring guards from the `_in_str` twins. CheckpointManager rule marked forward-looking. Carries the **buffer-sizing-by-scope** rule (module 8N vs function N) that 1.9.8 established. |
| `CLAUDE.md` | 2026-09-23 | ✅ Fresh | 1.9.13: fmt gate updated; step 1 now says to `rm -rf lib && cyrius deps` at a pin bump; `approval.cyr` claim corrected. `cyrius check` corrected to walk the entry. Version-sync step now names `scripts/version-bump.sh` and the `VERSION_STR` literal. "Security first" no longer claims sandbox/approval execution. |
