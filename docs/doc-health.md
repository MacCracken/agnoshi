...
| `VERSION` | 2026-08-29 | ✅ Fresh | `1.9.2` — single source of truth, read into `cyrius.cyml` via `${file:VERSION}`. Bumped via `scripts/version-bump.sh`, which also syncs the `VERSION_STR` banner literal in `src/agnsh.cyr` (the 1.8.9 desync gate). |
