...
> **Last refresh**: 2026-09-26 — **incremental, against the 2.0.2 tree** (cyrius 6.6.6, agnos 1.57.9).
> Rows touched by 1.9.13 – 2.0.2 are updated below; the rest carry forward.
>
...
> **Scope**: this repo only — 7 root markdown files, 26 under `docs/` (ADR-007 and ADR-008 added
> in 1.9.16 / 2.0.0, two archived issue records in 2.0.1), plus the `agnsh.1` man page.
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `roadmap.md` | 2026-09-26 | ✅ Fresh | 2.0.2 shipped: the `security.cyr` slot is deleted and 2.0.3 (checkpointing) is next; 2.0.4 notes that `execute_with_privileges` launches through `exec_vec` (an empty environment, a second host launcher) and must move to `host_exec`; the backlog gains a `--restricted` flag. The 2.0.x standing note (agnos launchers exiting 1 where ADR-008 § 4 says 126 / 127) is gone — resolved, with a harness that reads an agnos `-c` status; the pin-bump checklist and the next-agent notes name both QEMU scripts as the agnos tests. 2.0.1 (unplanned — two agnos 1.57.9 issues) took the slot: the 2.0.x arc moved down by one (`security.cyr` is 2.0.2) and its patch-number citations with it. § Gated re-verified against agnos 1.57.9: all seven agnos rows had resolved upstream in 1.57.6 / 1.57.7 — four now sit in a new *Opened upstream — agnsh's half* table saying what is left for agnsh, loopback TCP became the cyrius socket-adapter gate, and the backlog's kernel-side arm clear (shipped in 1.57.6) is gone. The two-target paragraph no longer says the host's only exec path is `run`. A 2.0.x standing note lists the agnos launch paths that still exit 1 where ADR-008 § 4 says 126 / 127. 2.0.0 shipped: the arc renumbered from 1.10.x (ruled — the `-c` contract break makes NL exec a major), the 2.0.0 slot retired, next is 2.0.1; the old "2.0.0 — what would force a major" section is now 3.0.0's, minus the `-c` contract it delivered. 1.9.16 shipped and retired: **the 1.9.x arc is closed**. 2.0.x is next, gated on the NL exec contract ruling; 2.0.0 now carries the host's shell-first adoption (ADR-007 decisions 1–2, a `PATH` lookup) and the Open-decisions row for ADR-007 is gone. 1.9.14 and 1.9.15 shipped and retired before it. **Rewritten at 1.9.13 into version-pinned release arcs** — 1.9.x close-out, 2.0.x NL execution, 2.1.x interactive shell, 2.2.x hoosh/LLM — plus Open decisions, Gated (with the agnos issue each gate is filed as), a standing pin-bump checklist replacing the per-version pin sections, and a release checklist. Slots are cited as `roadmap <arc> — <title>`, never by patch number. Every gate re-verified at 6.6.6; three had opened (agnos verification via QEMU, agnos `AO_NOFOLLOW`, the host LLM client). |
| `issue/` | 2026-09-26 | 📦 Archive | Three archived issue records — the two agnos 1.57.9 filings (a pipeline that keeps its read end; poll-and-yield waits) resolved in 2.0.1, each with a Resolution section and its evidence. Nothing open. |
...
| File | Status | Notes |
|---|---|---|
| `README.md` | 🔵 Evergreen | The ADR index. 2.0.2: ADR-008's line notes its amendment. (Until 2.0.2 this row carried the root README's notes.) |
...
| `008-nl-exec-contract.md` | ✅ Fresh + amended | 2.0.2: **amended** (ruled 2026-09-26) — § 1 gains restricted sessions (as root on a Linux host no NL line runs), § 4's 126 row names them, § 5 their `proposed` + `denied` records; a dated correction under § 4 says the agnos `&`, `\|` and `>` launchers followed the table only from 2.0.2. 2.0.1: its two citations of the approval slot now name it by arc and title (the patch number moved when 2.0.1 took the slot). Accepted for 2.0.0. The three NL-exec rulings (reuse `run`'s mode policy; the `-c` report to a report folder — XDG on the host, `/.agnsh_reports` on agnos; the host reaches a program through one `$PATH` lookup), the typed-BLOCKED ruling, and what they imply: `--dry-run` runs nothing, the 126 / 127 table, the USER_WRITE audit-label correction, argv-only launches, 2.0.0 as a major. |
| `007-input-classification.md` | ✅ Fresh | 2.0.1: a dated correction under decision 1 — agnos's "does this already" did not hold for a sentence with `$`, `(`, `)`, `;` or `..`, which its launcher refused before any lookup; fixed in the code, and the note says so. Its 1.10.x references now read 2.0.x (the arc was renumbered by ADR-008's versioning ruling). Accepted 1.9.16. Shell first on both targets (host `PATH` lookup in 2.0.0); `\|` / `>` are shell syntax, never NL; specific-before-broad with anchored triggers, the file-verb rule and anchored git; enforced by `tests/test_parse_corpus.tcyr`. Carries its evidence (13/161 regressions for the naive reorder, 104/189 for a bare-word-only verb rule) and its cost (the PhraseIndex, and the per-bench deltas against 1.9.15). Corrected after the tag: its benchmark figures came from an intermediate build and now quote the tagged code's (within 3 points of the old ones). |
...
an ADR are the approval-vs-execute split (roadmap 2.0.x — approval-gated exec) and the power-verb ruling
(§ Open decisions).
...
| File | Last touched | Status | Notes |
|---|---|---|---|
| `README.md` | 2026-09-26 | 🔵 Evergreen | The examples index — three links, all present. (Until 2.0.2 this row carried the root README's notes.) |
...
| `scripting.md` | 2026-09-26 | ✅ Fresh | 2.0.2: the exit-status table's 126 row names a restricted session; the table now holds for agnos's `&`, `\|` and `>` launchers too (1 through 2.0.1). **Rewritten for 2.0.0**: the `-c` contract (what runs, the report folder, `--dry-run`, the exit-status table), a Migrating-from-1.x table, aliases for run vs classify, and the audit labels (USER_WRITE is `needs_approval`). 1.9.15: the `-c` output format shows the corrected HIGH-risk / BLOCKED lines. stdout/stderr split documented (1.9.4). Audit-log schema updated with `exit_code` and the two label sets; added `jq` recipes for "what actually ran" and for detecting a `launched` with no outcome. |
| `server-hardening.md` | 2026-09-26 | 🟠 Aspirational, clearly marked | 2.0.2: the banner is re-verified against 2.0.2 — ⚠ it said `--strict` exits 0 (it exits 1) and that strict mode prompts for nothing (it confirms every launch since 2.0.0); the restricted row and section say what exists (automatic, root only) and that `--restricted` is not a flag. 2.0.0: the strict-mode note and the Layered Defense table say what 2.0.0 runs. 1.9.13: citations moved to arc slots. ⛔ Opens with a **do-not-deploy banner** and a table of its five false claims vs reality. Kept rather than deleted because it is a legitimate design target for the Bucket 1 exec + approval slices — but it is no longer readable as instructions. |
