...
- **v1.4.0 – v1.8.9** (2026-06-03 → 2026-08-07) — ⚠ **not back-filled here.** Fifteen releases shipped without a Shipped-list row: the agnos exec/spawn arc (env inheritance, PTY, job control, pipe streaming, socket + desktop-launch repairs), the 6.1.x/6.2.x/6.3.x toolchain bumps, and the 1.8.9 version-literal gate. `CHANGELOG.md` carries all fifteen in full and remains the source of truth for shipped work; this list is the stale surface. **Back-filling is its own slice** — it was deliberately not folded into the v1.9.0 toolchain bump.
- **v1.9.0** (2026-08-29) — Cyrius toolchain pin 6.3.34 → **6.5.36** + full `./lib/` re-sync to the 6.5.36 stdlib snapshot. The pin had drifted three minor lines behind an already-6.5.36 wrapper, so CI (which installs from the manifest) was the only place still on 6.3.34; aligning it surfaced **two tooling regressions hiding under green-looking gates**. (a) **CI's format gate could not pass**: `cyrius fmt <file>` rewrites *in place* as of 6.5.x and prints nothing, but the gate diffed the committed file against its stdout — empty stdout means every file reads as drifted, and the same command silently reformats the CI checkout. Switched to `cyrius fmt --check` (a real non-mutating check in 6.5.x; the CI comment claiming it was a no-op dated back to 5.9.x). (b) **`bench-history.sh` silently recorded 4 of 10 benchmarks**: 6.5.x prints decimal averages (`2.429us`) and the parser matched integers only, so every microsecond row fell through to `continue` and was dropped while the run still exited 0 — the CSV this project calls its proof would have quietly hollowed out. Now parses floats, scales to integer ns, and **fails loud** on any unparseable `avg` line. Whole-tree reformat under 6.5's continuation-indent rule (26 files, provably formatting-only: `git diff -w` empty, 430 insertions / 430 deletions). Bundled libs bumped (`mabda` 3.0.1→4.1.0, `vani` 0.9.3→1.2.2, `sigil` 3.7.7→3.12.14, `sandhi` 1.4.4→1.9.14, `patra` 1.10.3→1.13.10, `sankoch` 2.2.5→2.7.10, `yukti` 2.2.3→2.3.8, `niyama` 1.0.2→1.0.7; new: `bayan`, `ganita`, `yantra`) — both major bumps sit outside agnsh's include graph. Binary x86_64 282,016 → **311,352 B** (+10.4%), aarch64 478,464 → **544,688 B** (+13.8%), toolchain-side. ⚠ **Benchmarks are not comparable across this boundary** — 6.5.x subtracts a measured clock-read floor (1.328us) from every sample, so the apparent 1.458us → 78ns on `sanitize/basename` is a measurement change; floor-adjusted, all ten sit within run-to-run jitter and **no performance win is claimed**. Gates clean; tests stay 301 + 26 + 59. Full detail: `CHANGELOG.md` 1.9.0 entry.
...
### Systems features
- **`>` output redirection — `cmd > file` (truncate) ✅ SHIPPED in agnoshi 1.8.3** (`sh_run_redirect` + `exec_redirect#62` to a file fd; `agnos/scripts/redirect-smoke.py` green; adversarially verified 5 dimensions). **Remaining lane:** `>>` append, `<` stdin, `2>` stderr; plus tracked low-severity hardening (symlink `AO_NOFOLLOW`, robust kernel one-shot-clear on `#37` failure, system-path write denylist — see agnoshi CHANGELOG 1.8.3 § Deferred). The natural next cmdln-parsing feature after `|` pipes (1.46.11). **The hard part is already done** — the kernel `exec_redirect#62` primitive (the same one the pipe path uses) redirects a child's fd 1 to *any* destination fd, and agnsh already opens files for writing (the 1.41.7 write-fd path). So it's a parser branch + a file-open, structurally identical to the pipe path: split on byte 62 like it splits on 124 for `|`, `open(file, O_CREAT|O_WRONLY|O_TRUNC)`, `exec_redirect(1, fd)`, `execwait`, `close` — the pipe path opens a *pipe* fd as the dst, `>` opens a *file* fd instead. **Prioritized for the agnos 1.53.x (kernel FP/SIMD) window because it replaces FB-photo capture for iron diagnosis:** archaemenid has no serial, so today every iron result is a photograph — `klug > kernel.txt` after boot captures dropped log lines to a file, and a test-harness run can redirect its output into `tests/` for a second capture pass. That directly de-risks the 1.53.x FP math-test iron burns (a verifiable file instead of a photo). First of a broader cmdln-parsing lane (`>>`, `<` stdin-from-file, `2>` stderr, globbing, …) to open as demand warrants.
- **Metacharacter pass-through in `human` mode (deferred, user-flagged 2026-07-07).** `human` mode is the explicit "agnsh steps out of the way, user owns the risk" mode — so blanket rejection of shell metacharacters (`;`, `|`, `&`, `$()`, backtick, `<`, `>` — see `is_shell_metachar` in `src/sanitize.cyr`) is more paternalistic than the mode intends. Let the standard Linux shell symbols through in **`human` mode only**, keeping the reject-all guard for `auto`/`assist`/`strict`. **Must surface a threat-risk warning on Linux targets** — there, metachar → `execve` pass-through is a real command-injection/damage vector (the reason they're rejected today; audit C2, `docs/audit/2026-04-13.md`); the agnos model is more contained. Structural operators agnsh parses itself (`|`, `>`) are unaffected — this is specifically about letting a human-mode user type raw shell syntax at their own risk.
...
- **Man page integration — two faces along the AI/human axis.** `explain <cmd>` (AI-shelled): the assist layer reads the man page and explains it in context / natural language, for the AI modes. `docs <cmd>` (human side): a direct raw man-page viewer, no AI — for `human` mode, or when you just want the manual, not an interpretation. Same man source, two front-ends — mirrors the human/AI split behind the [human-mode metacharacter](#systems-features) idea above.
