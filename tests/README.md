...
| `test_core.tcyr` | unit tests | mode manager, permissions, intent parsing, translators, approval matrix, audit log, security context and restricted sessions, the report folder, the host launcher, NL execution, the agnos exit-status mapping and the shared program-word check (939 checks) |
...
| `bench_core.bcyr` | benchmarks | parse / translate / permission / sanitize hot paths, and the per-start security context |
